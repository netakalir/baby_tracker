...
- **Read the NotebookLM inbox at the start of each work session**: before starting work in this project, read the Google Drive doc `baby-tracker – notes from NotebookLM (for Claude)` (Drive file id `1FWea6GSU_tgU7nKKBDk9LPUhWbuvjAhN1Mz3VsdV2_0`, in the "baby tracker" folder) via the Google Drive connector. It is a one-way inbox where Netanel pastes/exports notes from NotebookLM that Claude should be aware of. Only read it (never edit it); if it contains no new notes since last time, just proceed.
